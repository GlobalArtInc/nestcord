...
export type CustomThreadUpdateEvents = {
  ...
  threadRateLimitPerUserUpdate: [thread: ThreadChannel, oldRateLimit: number, newRateLimit: number];
  threadAutoArchiveDurationUpdate: [thread: ThreadChannel, oldDuration: number | string, newDuration: number | string];
};
...
export class ThreadUpdateHandler extends BaseHandler<CustomThreadUpdateEvents> {
  ...
  @CustomListenerHandler()
  public handleThreadAutoArchiveDurationUpdate([oldThread, newThread]: ContextOf<'threadUpdate'>) {
