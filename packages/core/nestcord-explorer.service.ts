...
import { NestCordBaseDiscovery, NestCordContextType, NestCordParamsFactory } from './context';
...
export class ExplorerService<T extends NestCordBaseDiscovery> extends Reflector {
	...
	private createContextCallback(instance: object, prototype: unknown, methodName: string) {
		return this.externalContextCreator.create<Record<number, ParamMetadata>, NestCordContextType>(
			instance,
			prototype[methodName],
			methodName,
			ROUTE_ARGS_METADATA,
			this.nestcordParamsFactory,
			STATIC_CONTEXT,
			undefined,
