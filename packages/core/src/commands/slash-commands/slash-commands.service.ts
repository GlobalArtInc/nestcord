...
export class SlashCommandsService {
  ...
  public addSubCommand(subCommand: SlashCommandDiscovery): void {
    let rootCommand = this.reflector.get<SlashCommandDiscovery>(SlashCommand.KEY, subCommand.getClass());
    let subCommandGroup = this.reflector.get<SlashCommandDiscovery>(SubcommandGroup.KEY, subCommand.getClass());
